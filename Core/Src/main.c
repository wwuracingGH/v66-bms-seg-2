...
int main(void)
{
  ...
  //BLESpiInit(&hspi2);					// Pass peripheral pointers to start bluetooth SPI
  ...
  while(1) {
	  ...
	  for (int i = 0; i < NUMTHERMISTORS; i++) {
		  ...
		  if ((10 * (uint16_t) cellTemps[0][i] > highestTemp) && (cellTemps[0][i] != 0)) {
			  // New highest cell temp found, update value
			  highestTemp = 10 * (uint16_t) cellTemps[0][i];
		  }

		  if ((10 * (uint16_t) cellTemps[0][i] < lowestTemp) && (cellTemps[0][i] != 0)) {
			  // New lowest cell temp found, update value
			  lowestTemp = 10 * (uint16_t) cellTemps[0][i];
		  }
		  ...
		  if ((10 * (uint16_t) cellTemps[1][i] > highestTemp) && (cellTemps[1][i] != 0)) {
			  // New highest cell temp found, update value
			  highestTemp = 10 * (uint16_t) cellTemps[1][i];
		  }

		  if ((10 * (uint16_t) cellTemps[1][i] < lowestTemp) && (cellTemps[1][i] != 0)) {
			  // New lowest cell temp found, update value
			  lowestTemp = 10 * (uint16_t) cellTemps[1][i];
		  }

		  // Add reading to sum for averaging later
		  sumTemp = sumTemp + cellTemps[0][i] + cellTemps[1][i];
	  }
